...
| **SS2015 §3, Eqs. (38a–d)** | **δBGE-fit** (δ Beta-Gamma-Epsilon fit) — vector extension of Hu et al. δ-fit. Eqs. (38a–d) cover **only the diagonal families**: `β*_l = c_l/c₀`, and `δ*_l, α*_l, ζ*_l = (x_l − (β_l − c_l))/c₀`. The `(β_l − c_l)` peak subtraction is diagonal-only because the Dirac spike has `γ^δ_l = ϵ^δ_l = 0` (S2014 A.5–A.10). | `src/Scattering/truncate_phase.jl` (`truncate_phase(::δBGE, …)`, `truncate_phase_lowconf`), `truncate_phase_lin.jl` |
| **SS2015 §3, Eqs. (39)–(47)** | The off-diagonal δBGE fits: normal equations `A^g x^g = b^g`, `A^e x^e = b^e` for the coefficients `g_l`, `e_l` fitted to the **untruncated** `b_1(μ)`, `b_2(μ)`. ⚠ **The paper never writes the final normalisation of `g_l`/`e_l` into `γ*_l`/`ϵ*_l` explicitly — it is textually ambiguous at this point.** It is however fixed unambiguously by Eq. (8) (`Z* = (Z − f_tr·δ·E)/(1 − f_tr)`, i.e. *every* family divided by `c₀`) and by the δ-m analogue Eqs. (27e,f), which read `γ^n_l = γ_l/(1 − f_tr)`, `ϵ^n_l = ϵ_l/(1 − f_tr)` — pure `1/c₀`, no peak subtraction. So `γ*_l = g_l/c₀`, `ϵ*_l = e_l/c₀`. Omitting the `1/c₀` dilutes the truncated degree of linear polarisation `−f₁₂/f₁₁` by exactly `c₀`. | `src/Scattering/truncate_phase.jl`, `truncate_phase_lin.jl` (the `γᵗ = γᵗ_fit / c₀` lines) |
