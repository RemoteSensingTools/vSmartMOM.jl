...
"""
    get_elem_rt_RRS!(fscattRayl, ϖ_λ₁λ₀, i_λ₁λ₀, i_ref, ier⁻⁺, iet⁺⁺,
                     dτ_λ, ϖ_λ, Z⁻⁺_λ₁λ₀, Z⁺⁺_λ₁λ₀, qp_μN, wct2)

KernelAbstractions elemental R/T kernel for rotational Raman scattering. Each
workitem owns one stream pair, output wavelength `n₁`, and Raman offset `Δn`;
it maps to the incident wavelength `n₀`, skips out-of-band couplings, and
writes inelastic reflection/transmission using the finite-δ wavelength-coupled
single-scattering formulas.
"""
@kernel function get_elem_rt_RRS!(@Const(fscattRayl),
                            @Const(ϖ_λ₁λ₀), @Const(i_λ₁λ₀), i_ref,
                            ier⁻⁺, iet⁺⁺,
                            @Const(dτ_λ), @Const(ϖ_λ),
    ...
    n₀  = n₁ + i_λ₁λ₀[Δn]
    FT = eltype(ier⁻⁺)
    ier⁻⁺[i,j,n₁,Δn] = zero(FT)
    iet⁺⁺[i,j,n₁,Δn] = zero(FT)


    if (1 <= n₀) & (n₀ <= nMax) & (wct2[j] > rt_weight_tol(eltype(wct2)))
        ...
        if (qp_μN[i] == qp_μN[j])
            ...
            if (abs( (qp_μN[i]/qp_μN[j]) - (dτ_λ[n₁]/dτ_λ[n₀]) ) < rt_close_tol(eltype(dτ_λ)))
                iet⁺⁺[i,j,n₁,Δn] =
                ...
                if (i%3==1)&(j%3==1)&(n₁==200)
                    @show i, j, n₁, n₀, Δn
                        ...
                        @show expdiff_neg(dτ_λ[n₁] / qp_μN[i], dτ_λ[n₀] / qp_μN[j])
                        @show wct2[j]
                        @show iet⁺⁺[i,j,n₁,Δn]
                 end
                 =#
...
"""
    get_elem_rt_VS!(fscattRayl, ϖ_λ₁λ₀, i_λ₁λ₀, i_ref, ier⁻⁺, iet⁺⁺,
                    dτ_λ, ϖ_λ, Z⁻⁺_λ₁λ₀, Z⁺⁺_λ₁λ₀, qp_μN, wct2)

KernelAbstractions elemental R/T kernel for vibrational Raman scattering. Each
workitem owns one stream pair and Raman offset, maps the fixed incident
wavelength to the target wavelength, and writes the inelastic reflection and
transmission entries for active quadrature weights.
"""
@kernel function get_elem_rt_VS!(@Const(fscattRayl),
    ...
    if (wct2[j] > rt_weight_tol(eltype(wct2)))
        ...
        if (qp_μN[i] == qp_μN[j])
            ...
            # 𝐓⁺⁺(μᵢ, μᵢ) = (exp{-τ/μᵢ} + ϖ ̇𝐙⁺⁺(μᵢ, μᵢ) ̇(τ/μᵢ) ̇exp{-τ/μᵢ}) ̇𝑤ᵢ
            if i == j
                if abs(dτ_λ[n₀]-dτ_λ[n₁]) > rt_close_tol(eltype(dτ_λ))
                    iet⁺⁺[i,j,n₁,1] =
...
"""
    get_elem_rt_SFI_RRS!(fscattRayl, ϖ_λ₁λ₀, i_λ₁λ₀, i_ref, ieJ₀⁺, ieJ₀⁻,
                         τ_sum, dτ_λ, ϖ_λ, Z⁻⁺_λ₁λ₀, Z⁺⁺_λ₁λ₀, F₀,
                         qp_μN, ndoubl, wct02, nStokes, I₀, iμ0, D)

KernelAbstractions elemental source-function kernel for rotational Raman
couplings. Each workitem owns one stream, output wavelength, and Raman offset;
it forms the incident-wavelength `Z * F₀` contractions, applies finite-δ
source formulas, attenuates by the optical depth above the layer, and applies
the upwelling D-matrix sign when needed.
"""
@kernel function get_elem_rt_SFI_RRS!(@Const(fscattRayl),
                            @Const(ϖ_λ₁λ₀), @Const(i_λ₁λ₀), i_ref,
                            ieJ₀⁺, ieJ₀⁻,
                            @Const(τ_sum), @Const(dτ_λ), @Const(ϖ_λ),
    ...
    if (1 <= n₀) & (n₀ <= nMax)
        ...
        if (i_start <= i) & (i <= i_end)
                ...
                ieJ₀⁺[i, 1, n₁, Δn] =
                        ...
                        if (i%3==1)&(n₁==200)
                            @show i, n₁, n₀, Δn
                                ...
                                @show expdiff_neg(dτ_λ[n₁] / qp_μN[i], dτ_λ[n₀] / qp_μN[i])
                                #@show wct2[j]
                                @show ieJ₀⁺[i, 1, n₁, Δn]
                         end
                        =#
            ...
            # J₀⁺ = 0.25*(1+δ(m,0)) * ϖ(λ) * Z⁺⁺ * I₀ * [μ₀ / (μᵢ - μ₀)] * [exp(-dτ(λ)/μᵢ) - exp(-dτ(λ)/μ₀)]
            if (abs( (qp_μN[i]/qp_μN[i_start]) - (dτ_λ[n₁]/dτ_λ[n₀]) ) < rt_close_tol(eltype(dτ_λ)))
                ieJ₀⁺[i, 1, n₁, Δn] =
...
#=
                    ...
                    @show i, n₁, n₀, Δn
                                ...
                                @show expdiff_neg(dτ_λ[n₁] / qp_μN[i], dτ_λ[n₀] / qp_μN[i_start])
                                #@show wct2[j]
                                @show ieJ₀⁺[i, 1, n₁, Δn]
                         end=#
        end
...
"""
    apply_D_elemental_RRS!(ndoubl, pol_n, ier⁻⁺, iet⁺⁺, ier⁺⁻, iet⁻⁻)

KernelAbstractions D-matrix symmetry kernel for rotational Raman elemental
R/T operators. It either fills reverse operators for undoubled elemental
layers or applies the row sign to `ier⁻⁺` before subsequent doubling.
"""
@kernel function apply_D_elemental_RRS!(ndoubl, pol_n, ier⁻⁺, @Const(iet⁺⁺), ier⁺⁻, iet⁻⁻)
    i, j, n₁, Δn = @index(Global, NTuple)
...
"""
    apply_D_elemental_VS!(ndoubl, pol_n, i_λ₁λ₀, ier⁻⁺, iet⁺⁺, ier⁺⁻, iet⁻⁻)

KernelAbstractions D-matrix symmetry kernel for vibrational Raman elemental
R/T operators. Each workitem maps a Raman offset to its active wavelength,
skips inactive offsets, and applies the same Stokes parity signs as the
elastic elemental D-matrix kernel.
"""
@kernel function apply_D_elemental_VS!(ndoubl,
                                pol_n,
                                @Const(i_λ₁λ₀),
                                ier⁻⁺, @Const(iet⁺⁺), ier⁺⁻, iet⁻⁻)
...
"""
    apply_D_elemental_SFI_RRS!(ndoubl, pol_n, ieJ₀⁻)

KernelAbstractions D-matrix source-vector kernel for rotational Raman
elemental layers. It negates upwelling `U/V` source entries when the
source-vector symmetry has not already been applied by doubling.
"""
...
"""
    apply_D_elemental_SFI_VS!(ndoubl, pol_n, i_λ₁λ₀, ieJ₀⁻)

KernelAbstractions D-matrix source-vector kernel for vibrational Raman
elemental layers. It maps active Raman offsets to wavelengths and negates the
upwelling `U/V` source entries in place.
"""
