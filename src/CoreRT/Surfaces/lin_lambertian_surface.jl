...
function create_surface_layer!(lambertian::LambertianSurfaceScalar{FT}, 
                               ...
                               lin_added_layer::Union{linAddedLayer,linAddedLayerRS},
                               SFI,
                               m::Int,
                               pol_type,
                               quad_points,
    ...
    if m == 0
        ...
        if SFI
            ...
            added_layer.j₀⁺[:,1,:] .= 0; #I₀_NquadN .* exp.(-τ_sum/μ₀)';
            added_layer.j₀⁻[:,1,:] .= μ₀*(R_surf*I₀_NquadN) .* exp.(-τ_sum/μ₀)';
            for ctr=1:Nparams
                if ctr==i_surf
                    lin_added_layer.dxj₀⁺[ctr,:,1,:] .= 0;#I₀_NquadN .* exp.(-τ_sum/μ₀)'/lambertian.ρ;
        ...
        lin_added_layer.dxr⁻⁺ .= 0;
        lin_added_layer.dxr⁺⁻ .= 0;
        lin_added_layer.dxt⁺⁺ .= 0;
        lin_added_layer.dxt⁻⁻ .= 0;
        lin_added_layer.dxr⁻⁺[i_surf,:,:,:] .= R_surf./lambertian.ρ;
...
function create_surface_layer!(lambertian::LambertianSurfaceLegendre{FT}, 
    ...
    τ_sum,
    lin_τ_sum,
    Nparams, i_surf, #here i_surf is an array of indices
    architecture) where {FT}
    FT2 = Float64
    if m == 0
        @unpack qp_μ, wt_μ, qp_μN, wt_μN, iμ₀Nstart, iμ₀, μ₀ = quad_points
        ...
        if SFI
            ...
            for ctr=1:Nparams
                if ctr∈i_surf
                    lin_added_layer.dxj₀⁺[:,1,:] .= 0;#I₀_NquadN .* exp.(-τ_sum/μ₀)'/lambertian.ρ;
        ...
        lin_added_layer.dxt⁺⁺ .= 0;
        lin_added_layer.dxt⁻⁻ .= 0;
        for ctr in i_surf #all elements in i_surf are assumed to be contiguous, e.g. 10, 11, 12
            lin_added_layer.dxr⁻⁺[ctr,:,:,:] .= reshape(reduce(hcat,[i*R_surf for i in Array(P[:,ctr-i_surf[1]+1])]), siz...);
        end
    else
        added_layer.r⁻⁺[:] .= 0;
        added_layer.r⁻⁺[:] .= 0;
