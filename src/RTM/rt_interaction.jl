...
function rt_interaction!(mo::Stokes_IQUV, kn, n, m)
    ...
    if kn==1
        ...
        for iμ in 1:Nquad4
            T_mm[iμ, iμ] = t_elt_mm[iμ, iμ]*T_mm[iμ, iμ]
            T_pp[iμ, iμ] = t_elt_pp[iμ, iμ]*T_pp[iμ, iμ]
        end
        return nothing
    elseif kn==2
    ...
    elseif kn==4
        ...
        # added to the bottom of the composite layer.
        # Produces a new, scattering composite layer.
        M1 = inv(I - R_pm * r_elt_mp)
        t_R_mp = R_mp + T_mm * r_elt_mp * M1 * T_pp
        t_T_pp = t_elt_pp * M1 * T_pp
        #repeating for mirror-reflected directions
        M1 = inv(I - r_elt_mp * R_pm)
        t_R_pm = r_elt_pm + t_elt_pp * R_pm * M1 * t_elt_mm
        t_T_mm = T_pp * M1 * t_elt_mm
        return nothing
    end
end