    ...
    if scatter
        ...
        # This is not yet GPU ready as it has element wise operations (should work for CPU)
            ...
            for i = 1:NquadN, j=1:NquadN
                ...
                r⁻⁺[i,j,:] = ϖ_nSpec .* Z⁻⁺[i,j] .* (qp_μ4[j]/(qp_μ4[i]+qp_μ4[j])) .* (1 .- exp.(-dτ_nSpec .* ((1/qp_μ4[i])+(1/qp_μ4[j])))) .* (wct[j]) 

                if (qp_μ4[i]==qp_μ4[j])

                    # 𝐓⁺⁺(μᵢ, μᵢ) = ϖ ̇𝐙⁺⁺(μᵢ, μᵢ) ̇(τ/μᵢ) ̇exp{-τ/μᵢ} ̇𝑤ᵢ
                    t⁺⁺[i,j,:] = ϖ_nSpec .* Z⁺⁺[i,i] .* (dτ_nSpec ./ qp_μ4[i]) .* exp.(-dτ_nSpec./qp_μ4[i]) .* wct[i]

                else

                    # 𝐓⁺⁺(μᵢ, μⱼ) = ϖ ̇𝐙⁺⁺(μᵢ, μⱼ) ̇(μⱼ/(μᵢ-μⱼ)) ̇(exp{-τ/μᵢ} - exp{-τ/μⱼ}) ̇𝑤ⱼ
                    # (𝑖 ≠ 𝑗)
                    t⁺⁺[i,j,:] = ϖ_nSpec .* Z⁺⁺[i,j] .* (qp_μ4[j]/(qp_μ4[i]-qp_μ4[j])) .* (exp.(-dτ_nSpec ./qp_μ4[i]) - exp.(-dτ_nSpec./qp_μ4[j])) .* wct[j]
                end
            end
