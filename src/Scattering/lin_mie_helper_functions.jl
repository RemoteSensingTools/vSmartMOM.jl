...
The function returns a rounded integer, following conventions by BH, Rooj/Stap, Siewert 
"""
function compute_mie_ab!(size_param, refractive_idx::Number, an, bn, Dn, dan, dbn, dDn)
    # Compute y
    y = size_param * refractive_idx
    dy = size_param # derivative with respect to nᵣ. The derivative with respect to nᵢ will be -i⋅dy
    ...
    [dDn[n] = (-(n+1) / y^2)*dy + (1 / (Dn[n+1] + (n+1) / y))^2 * (dDn[n+1] - (n+1)*dy/y^2) for n = (nmx - 1):-1:1]
    ...
    for n = 1:n_max  
        ...
        dt_a = dDn[n] * refractive_idx
        dt_b = dt_a
        dt_a -= Dn[n]
        dt_a /= refractive_idx^2
        dt_b += Dn[n]
...
function compute_anbn(model::MieModel, λ, radius)
    ...
    dan = zeros(Complex{FT2}, nquad_radius, N_max)
    dbn = zeros(Complex{FT2}, nquad_radius, N_max)
    ...
    @inbounds for n in 1:Nmax, m in n:Nmax
        ...
        @inbounds for i = 1:size(an, 1)
            if m < N_max_[i] && n < N_max_[i]
                ...
                danam += w[i] * (an[i,n]' * dan[i,m] + dan[i,n]' * an[i,m])
                dbnbm += w[i] * (bn[i,n]' * dbn[i,m] + dbn[i,n]' * bn[i,m])
                danbm += w[i] * (an[i,n]' * dbn[i,m] + dan[i,n]' * bn[i,m])
                dbnam += w[i] * (bn[i,n]' * dan[i,m] + dbn[i,n]' * an[i,m])
            end
        end 
        ...
        @inbounds mat_danam[m,n] = danam;
        @inbounds mat_dbnbm[m,n] = dbnbm;
        @inbounds mat_danbm[m,n] = danbm;
        @inbounds mat_dbnam[m,n] = dbnam;
    end
end
...
"""
function compute_mie_S₁S₂!(an, bn, dan, dbn, π_, τ_, S₁, S₂, dS₁, dS₂)
    FT = eltype(an)
    nmax = size(an, 1);
    nμ   = size(π_, 1);
    ...
    for l in 1:nmax, iμ in 1:nμ 
            ...
            dS₁[iμ] += (2l + 1) / (l * (l + 1)) * (dan[l] * τ_[iμ,l] + dbn[l] * π_[iμ,l])
            dS₂[iμ] += (2l + 1) / (l * (l + 1)) * (dan[l] * π_[iμ,l] + dbn[l] * τ_[iμ,l])
    end
end
...
"""
function reconstruct_phase(greek_coefs, dgreek_coefs, μ; returnLeg=false)
...
""" 
function compute_avg_C_scatt_ext(k, an, bn, dan, dbn, w)
