...
"""
function truncate_phase(mod::δBGE, aero::AerosolOptics{FT}, lin::LinAerosolOptics{FT}; reportFit=false) where {FT}
    ...
    #= for dβ
       Ax=b, where
       Aᵢⱼ = ∑ₖ w_μₖ Pᵢ(μₖ)Pⱼ(μₖ)/f₁₁²(μₖ), xᵢ=dcᵢ (as in Sanghavi & Stephens 2015), and
    ...
    for i = 1:l_tr
        b[i] = sum(((2./f₁₁).*sum(cl.*P[:,:]) .- 1)w_μ.*P[:,i].*df₁₁[ctr,:]./f₁₁)
        #A[i,i] = sum(w_μ.*(P[:,i]./f₁₁).^2)
        for j = i+1:l_tr
            A[i,j] = sum(w_μ.*P[:,i].*P[:,j]./(f₁₁.^2))
            A[j,i] = A[i,j]
...
function truncate_phase(mod::δBGE, aero::AerosolOptics{FT}; lin::LinAerosolOptics{FT}, reportFit=false) where {FT}
    ...
    # W₁₂ = Diagonal(w_μ[iμ]);
    # W₃₄ = Diagonal(w_μ[iμ]);
    # Julia backslash operator for SVD (just like Matlab);
