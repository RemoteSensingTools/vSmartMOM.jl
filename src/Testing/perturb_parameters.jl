...
function perturb_parameters(params::vSmartMOM.CoreRT.vSmartMOM_Parameters, ppct)
    ...
    # BRDF
    # Adapted from sanghavi: unified's surface types (LambertianSurfaceScalar,
    # RossLi, RPV, Cox-Munk, Canopy) are immutable structs, so we reconstruct
    # rather than mutate the albedo field in place.
    for iband = 1:Nbands
        tmp_params = deepcopy(params)
        brdf_old = tmp_params.brdf[iband]
        albedo_val = brdf_old.albedo
        new_albedo = (albedo_val == 0) ? oftype(albedo_val, 0.01) : incr_fct * albedo_val
        tmp_params.brdf[iband] = _perturb_brdf_albedo(brdf_old, new_albedo)
        push!(pert_params, tmp_params)
    end
...
# Per-surface-type rebuild with a perturbed albedo. Only LambertianSurfaceScalar
# is wired today (Phase1b acceptance script); add other surface types here as
# FD-Jacobian verification extends to them.
_perturb_brdf_albedo(b::vSmartMOM.CoreRT.LambertianSurfaceScalar, new_albedo) =
    vSmartMOM.CoreRT.LambertianSurfaceScalar(new_albedo)
_perturb_brdf_albedo(b, new_albedo) =
    error("perturb_parameters: albedo perturbation not implemented for " *
          "$(typeof(b)). Add an `_perturb_brdf_albedo` method in src/Testing/perturb_parameters.jl.")
