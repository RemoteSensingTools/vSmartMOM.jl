...
parameters = 
    parameters_from_yaml("/home/sanghavi/code/github/vSmartMOM.jl/test/test_parameters/WCO2_parameters_SIF_grid.yaml");
...
OCO = true
model      = model_from_parameters(OCO, parameters);
...
for isurf = 1:1 # 3:3 # 2:2 # 1:1 # 
    for iρ = 1:21 #1:15 #3 #1:15
        for iA = 1:14
            ...
            model2.obs_geom = CoreRT.ObsGeometry(parameters.sza, parameters.vza, parameters.vaz, parameters.obs_alt)
            ...
            model2.quad_points = CoreRT.rt_set_streams(parameters.quadrature_type, 
                                                parameters.l_trunc, 
                                                model2.obs_geom, 
                                                parameters.polarization_type, 
                                                array_type(parameters.architecture))
            ...
            ocoRnoRS = zeros(FT, n_cam, parameters.polarization_type.n, tot_nspec)
            ocoTnoRS = zeros(FT, n_cam, parameters.polarization_type.n, tot_nspec)            
            ...
            for iBand=1:n_bands
                ...
                parameters.brdf[iBand].albedo = ρ[iρ]
                spec_start = 1 #spec_end+1
                spec_end = length(ν) #spec_end += length(ν) - 2*n_overlap
                ...
                ocoRnoRS0, ocoTnoRS0, _, _ = CoreRT.rt_run_test(RS_type0, model,iBand);
                RnoRS0, TnoRS0, _, _ = CoreRT.rt_run_test(RS_type0, model2,iBand);
                ...
                ocoRnoRS[:,:,spec_start:spec_end] = ocoRnoRS0[:,:,(n_overlap+1):(end-n_overlap)]
            ...
            for vctr = 1:length(model.params.vaz)
                ...
                # With SIF
                #fname0 = "/home/sanghavi/data/RamanSIFgrid/OCO/OCOraylSIF_sza"*sza_str*"_vza"*vza_str*"_vaz"*vaz_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_nors_WCO2.dat"
                ##fname1 = "/home/sanghavi/data/RamanSIFgrid/OCO/OCOraylSIF_sza"*sza_str*"_vza"*vza_str*"_vaz"*vaz_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_rrs_WCO2.dat"
                # No SIF
                oco_fname0 = "/home/sanghavi/data/RamanSIFgrid/OCO/OCOrayl_sza"*sza_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_nors_WCO2.dat"
                fname0 = "/home/sanghavi/data/RamanSIFgrid/OCO/rayl_sza"*sza_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_nors_WCO2.dat"
                #fname0 = "/home/sanghavi/data/RamanSIFgrid/OCO/OCOrayl_sza"*sza_str*"_vza"*vza_str*"_vaz"*vaz_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_nors_WCO2.dat"
                #fname1 = "/home/sanghavi/data/RamanSIFgrid/OCO/OCOrayl_sza"*sza_str*"_vza"*vza_str*"_vaz"*vaz_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_rrs_WCO2.dat"
                ...
                oco_rayl_nors_WCO2 = [tot_ν ocoRnoRS[vctr,1,:] ocoRnoRS[vctr,2,:] ocoRnoRS[vctr,3,:] tot_F₀]
                rayl_nors_WCO2 = [tot_ν RnoRS[vctr,1,:] RnoRS[vctr,2,:] RnoRS[vctr,3,:] tot_F₀]

                writedlm(oco_fname0, oco_rayl_nors_WCO2)
                writedlm(fname0, rayl_nors_WCO2)
