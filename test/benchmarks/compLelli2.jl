...
parameters = 
    parameters_from_yaml("test/test_parameters/ParamsLelliFig2.yaml");
...
#Rayleigh case:
#=
rayl_rrs = [ν R[1,1,:] R[1,2,:] R[1,3,:] ieR[1,1,:] ieR[1,2,:] ieR[1,3,:]]
rayl_nors = [ν RnoRS[1,1,:] RnoRS[1,2,:] RnoRS[1,3,:]]

writedlm("out_ray_rrs_385nm_405nm.dat", rayl_rrs)
writedlm("out_ray_nors_385nm_405nm.dat", rayl_nors)
=#
#Aerosol case:
aer_rrs = [ν R[1,1,:] R[1,2,:] R[1,3,:] ieR[1,1,:] ieR[1,2,:] ieR[1,3,:]]
aer_nors = [ν RnoRS[1,1,:] RnoRS[1,2,:] RnoRS[1,3,:]]

writedlm("out_aer_rrs_385nm_405nm.dat", aer_rrs)
writedlm("out_aer_nors_385nm_405nm.dat", aer_nors)
#=
...
x = -40:0.6:40
#kernel = InstrumentOperator.create_instrument_kernel(Normal(0, 12.5), x) #defining a Gaussian kernel for convulution in wavenumber space
kernel = InstrumentOperator.create_instrument_kernel(Normal(0, 7.5), x)
#I_conv = InstrumentOperator.conv_spectra(kernel, )
...
p1 = plot(1e7./ν, (rQ_conv_noRS)./(rI_conv_noRS), linewidth = 3, linecolor=:black, label="Rayleigh", xlims=[391,400])
p1 = plot!(1e7./ν, (rQ_conv+ierQ_conv)./(rI_conv+ierI_conv), linewidth = 3, linecolor=:red, label="")
p1 = plot!(1e7./ν, aQ_conv_noRS./aI_conv_noRS, linewidth = 3, linecolor=:lightgrey, label="Aerosol")
...
plot(p1, title= "Zenith obs, θ₀=63ᵒ, A=0.05", legend = :bottom)
