...
FT = Float64
Fraunhofer=true
# Calcium II H & K lines occur near 396.8 nm (K) and 393.4 nm (H)
λ₀ = 396.8 #745. #400. #nm
vmr_h2 = 0.1 #Atmospheric models of sunspot umbrae predict that the molecular hydrogen population can reach up to 10%, peaking near the height of continuum formation.
#iBand = 1

# Determine the effect of temperature on VRS
effT = [3500., 1500., 10.] # K
h2 = []
RS_type_h2 = []
σ_Rayl_h2  = []
...
ϖ_VS = []
for i=1:length(effT)
    push!(h2, InelasticScattering.getRamanSolarConstants(1.7/λ₀, effT[i], vmr_h2));
    push!(RS_type_h2, InelasticScattering.sol_VS_0to1_plus{FT}(
                   h2=h2[i]));
    InelasticScattering.getRamanSSProp!(RS_type_h2[i], λ₀);
    push!(σ_Rayl_h2, compute_stellar_Rayl(λ₀, h2[i]))
    λ_tmp = 1e7./RS_type_h2[i].grid_in[2][RS_type_h2[i].i_λ₁λ₀]
    ϖ_tmp = RS_type_h2[i].ϖ_λ₁λ₀
    push!(λ_VS, λ_tmp)
    push!(ϖ_VS, ϖ_tmp)
end


#h2 = InelasticScattering.getRamanSolarConstants(1.7/λ₀, 3500., vmr_h2);
