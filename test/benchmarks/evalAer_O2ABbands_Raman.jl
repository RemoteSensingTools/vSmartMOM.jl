...
F₀_BB = reverse(aer_nors_BBO2[:,5]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
...
# Rayleigh case: B-band
#fname0 = "/home/sanghavi/data/RamanSIFgrid/aerosol_test/aer_AOD0p0_sza0_vza0p0_vaz0p0_alb0p0_psurf1000hpa_nors_ABO2.dat"
#rayl_nors_ABO2 = unique(readdlm(fname0), dims=1)
    
#ν = rayl_nors_ABO2[:,1]
#λ = 1e7./reverse(ν)
I0rrs_BB = zeros(length(ρ),length(sza),length(λ_BB))
Q0rrs_BB = zeros(length(ρ),length(sza),length(λ_BB))
U0rrs_BB = zeros(length(ρ),length(sza),length(λ_BB))
ieI0rrs_BB = zeros(length(ρ),length(sza),length(λ_BB))
ieQ0rrs_BB = zeros(length(ρ),length(sza),length(λ_BB))
ieU0rrs_BB = zeros(length(ρ),length(sza),length(λ_BB))
#F₀ = reverse(aer_nors_ABO2[:,8]).* (1e7./λ.^2) #mW/m^2/s/nm
I0nors_BB = zeros(length(ρ),length(sza),length(λ_BB))
Q0nors_BB = zeros(length(ρ),length(sza),length(λ_BB))
U0nors_BB = zeros(length(ρ),length(sza),length(λ_BB))
...
for isurf = 1:1 # 3:3 # 2:2 # 1:1 # 
    for iρ = 1:length(ρ) #21 #1:15 #3 #1:15
        for iA = 1:length(sza) #14
            ...
            # No SIF
            fname0 = "/home/sanghavi/data/RamanSIFgrid/aerosol_test/aer_AOD0p0_sza"*sza_str*"_vza"*vza_str*"_vaz"*vaz_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_nors_BBO2.dat"
            fname1 = "/home/sanghavi/data/RamanSIFgrid/aerosol_test/aer_AOD0p0_sza"*sza_str*"_vza"*vza_str*"_vaz"*vaz_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_rrs_BBO2.dat"
                
            aer0_rrs_BBO2  = unique(readdlm(fname1), dims=1)
            aer0_nors_BBO2 = unique(readdlm(fname0), dims=1)
                
            #ν = aer_rrs_ABO2[:,1]
            #λ = 1e7./reverse(ν)
            I0rrs_BB[iρ,iA,:] = reverse(aer0_rrs_BBO2[:,2]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            Q0rrs_BB[iρ,iA,:] = reverse(aer0_rrs_BBO2[:,3]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            U0rrs_BB[iρ,iA,:] = reverse(aer0_rrs_BBO2[:,4]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            ieI0rrs_BB[iρ,iA,:] = reverse(aer0_rrs_BBO2[:,5]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            ieQ0rrs_BB[iρ,iA,:] = reverse(aer0_rrs_BBO2[:,6]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            ieU0rrs_BB[iρ,iA,:] = reverse(aer0_rrs_BBO2[:,7]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            #F₀ = reverse(aer_rrs_ABO2[:,8]).* (1e7./λ.^2) #mW/m^2/s/nm
            
            I0nors_BB[iρ,iA,:] = reverse(aer0_nors_BBO2[:,2]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            Q0nors_BB[iρ,iA,:] = reverse(aer0_nors_BBO2[:,3]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
            U0nors_BB[iρ,iA,:] = reverse(aer0_nors_BBO2[:,4]).* (1e7./λ_BB.^2) #mW/m^2/s/nm
...
ΔI0_BB = I0rrs_BB+ieI0rrs_BB-I0nors_BB
ΔQ0_BB = Q0rrs_BB+ieQ0rrs_BB-Q0nors_BB
ΔU0_BB = U0rrs_BB+ieU0rrs_BB-U0nors_BB

eΔI0_BB = I0rrs_BB-I0nors_BB
eΔQ0_BB = Q0rrs_BB-Q0nors_BB
eΔU0_BB = U0rrs_BB-U0nors_BB
...
ind0_hires = findall(x->x>fit_window0[1] && x<fit_window0[2], λ_BB);         
...
g = GridLayout(f[1, 1:6], nrows = 1, ncols = 6)
#g2 = GridLayout(f[1:8, 7:9], nrows = 1, ncols = 3)
Label(f[1, 1:6, Top()], "Effect of AOD vs. surface brightness",
        fontsize = 15,
        font = :bold,
        padding = (0, 0, 30, 0),
        halign = :center)
  
ax1 = Axis(g[1,1:2], title = "Window 0", 
        xlabel = L"$\lambda\,[\mathrm{nm}]$", 
        ylabel = L"$I_\mathrm{ie}\,$[mW/nm/sr/m$^2$]")  
ax2 = Axis(g[1,3:4], title = "Window 1", 
        xlabel = L"$\lambda\,[\mathrm{nm}]$", 
        ylabel = "")               
ax3 = Axis(g[1,5:6], title = "Window 2", 
        xlabel = L"$\lambda\,[\mathrm{nm}]$", 
        ylabel = "") 
# plots
ir=1 # r₀=0.1
...
for iρ=1:length(ρ)    
    iτ = 2 # AOD=0.5
    lines!(ax1, λ_BB[ind0_hires], ieI_BB[iτ,iz,ir,iρ,iA,ind0_hires], color=:indianred3, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    iτ = 1
    lines!(ax1, λ_BB[ind0_hires], ieI_BB[iτ,iz,ir,iρ,iA,ind0_hires], color=:mediumseagreen, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    lines!(ax1, λ_BB[ind0_hires], ieI0_BB[iρ,iA,ind0_hires], color=:royalblue1, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    text!(ax1, λ_BB[ind0_hires][50], minimum(ieI0_BB[iρ,iA,ind0_hires]), text = latexstring(L"$\rho=$"*latexstring(ρ[iρ])), align = (:right, :top), color = :black)

    iτ = 2 # AOD=0.5
    lines!(ax2, λ[ind1_hires], ieI[iτ,iz,ir,iρ,iA,ind1_hires], color=:indianred3, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    iτ = 1
    lines!(ax2, λ[ind1_hires], ieI[iτ,iz,ir,iρ,iA,ind1_hires], color=:mediumseagreen, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    lines!(ax2, λ[ind1_hires], ieI0[iρ,iA,ind1_hires], color=:royalblue1, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    text!(ax2, λ[ind1_hires][50], minimum(ieI0[iρ,iA,ind1_hires]), text = latexstring(L"$\rho=$"*latexstring(ρ[iρ])), align = (:right, :top), color = :black)
    
    iτ = 2 # AOD=0.5
    lines!(ax3, λ[ind2_hires], ieI[iτ,iz,ir,iρ,iA,ind2_hires], color=:indianred3, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    iτ = 1
    lines!(ax3, λ[ind2_hires], ieI[iτ,iz,ir,iρ,iA,ind2_hires], color=:mediumseagreen, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    lines!(ax3, λ[ind2_hires], ieI0[iρ,iA,ind2_hires], color=:royalblue1, linewidth=0.75,  label=L"$\mathrm{AOD}=$"*string(τ_ref[iτ])*L"$\,\,ρ=$"*string(ρ[iρ]))
    text!(ax3, λ[ind2_hires][10], minimum(ieI0[iρ,iA,ind2_hires]), text = latexstring(L"$\rho=$"*latexstring(ρ[iρ])), align = (:right, :top), color = :black)
end

legend_line01 = LineElement(color=:royalblue1) #lines!(ax1, λ[1:2], ΔI0[1,1,1:2],color=clist0[2],visible=false)
legend_line02 = LineElement(color=:mediumseagreen) #lines!(ax1, λ[1:2], ΔI0[1,1,1:2],color=clist0[3],visible=false)
legend_line03 = LineElement(color=:indianred3) #lines!(ax1, λ[1:2], ΔI0[1,1,1:2],color=clist0[4],visible=false)

legend_labels = [
    latexstring(L"$\mathrm{AOD}=0$"),
    latexstring(L"$\mathrm{AOD}=$" * latexstring(string(τ_ref[1]))),
    latexstring(L"$\mathrm{AOD}=$" * latexstring(string(τ_ref[2])))
]

legend_elements = [legend_line01, legend_line02, legend_line03]

legend = Legend(f, legend_elements, legend_labels, 
           orientation = :horizontal,  # Ensures elements are stacked properly
...
f[2, :] = legend
save("/home/sanghavi/data/RamanSIFgrid/aerosol_test/pics/ABRRScorr_wrt_AOD_z"*z_str[iz]*"_r"*r_str[ir]*"_sza"*string(sza[iA])*".png", f)
