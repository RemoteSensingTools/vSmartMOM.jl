...
# Code to generate absorption model from luts

folder0 = "/net/fluo/data3/data/Databases/Rami_CS_database/"
folder = "/home/sanghavi/O3_XS/"

o3 = readdlm(folder0*"serdyuchenkogorshelev5digits.dat", skipstart=45)
λ_grid    = o3[1,1]:0.01:o3[end,1]
# Remove <0 values:
o3[o3.<0] .= 0
...
itp_model =  InterpolationModel(etpf, mol, iso,  λ_grid, p_grid2, t_grid2)
...
parameters = 
    parameters_from_yaml("/home/sanghavi/code/github/vSmartMOM.jl/test/test_parameters/O3HugginsRaman.yaml");
    ...
    for iρ = 1:length(ρ) #15 #3 #1:15
        for iA = 1:length(sza)
            ...
            for iBand=1:n_bands
                ...
                T[:,:,spec_start:spec_end] = T1[:,:,(n_overlap+1):(end-n_overlap)]
                ieT[:,:,spec_start:spec_end] = ieT1[:,:,(n_overlap+1):(end-n_overlap)]
                TnoRS[:,:,spec_start:spec_end] = TnoRS0[:,:,(n_overlap+1):(end-n_overlap)]
            ...
            rayl_rrs_O3 = [tot_ν R[1,1,:] R[1,2,:] R[1,3,:] ieR[1,1,:] ieR[1,2,:] ieR[1,3,:] tot_F₀]
            rayl_nors_O3 = [tot_ν RnoRS[1,1,:] RnoRS[1,2,:] RnoRS[1,3,:] tot_F₀]
            #@show rayl_nors_ABO2
            albedo = ρ_str[iρ]
            sza_str = string(sza[iA])
            
            fname0 = "/home/sanghavi/O3RamanPolgrid/R_rayl_sza"*sza_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_nors_ABO2.dat"
            fname1 = "/home/sanghavi/O3RamanPolgrid/R_rayl_sza"*sza_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_rrs_ABO2.dat"
            writedlm(fname0, rayl_nors_O3)
            writedlm(fname1, rayl_rrs_O3)

            rayl_rrs_O3 = []
            rayl_nors_O3 = []
            
            rayl_rrs_O3 = [tot_ν T[1,1,:] T[1,2,:] T[1,3,:] ieT[1,1,:] ieT[1,2,:] ieT[1,3,:] tot_F₀]
            rayl_nors_O3 = [tot_ν TnoRS[1,1,:] TnoRS[1,2,:] TnoRS[1,3,:] tot_F₀]
            #@show rayl_nors_ABO2
            albedo = ρ_str[iρ]
            sza_str = string(sza[iA])
            
            fname0 = "/home/sanghavi/O3RamanPolgrid/T_rayl_sza"*sza_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_nors_ABO2.dat"
            fname1 = "/home/sanghavi/O3RamanPolgrid/T_rayl_sza"*sza_str*"_alb"*albedo*"_psurf"*string(psurf[isurf])*"hpa_rrs_ABO2.dat"
            writedlm(fname0, rayl_nors_O3)
            writedlm(fname1, rayl_rrs_O3)
        end
    end
