# =========================================================================
# O₂ A-band OCO-like Rayleigh grid driver (sanghavi Phase 6 port).
# =========================================================================
#
# Minimal single-scenario driver for the OCO-O2A Rayleigh+SIF grid. Uses
# `O2_parameters2_SIF_grid.yaml` (same spectral grid as the existing
# `creategrid_O2Aband_RamanSIF.jl` but intended for OCO LUT comparisons).
# The sanghavi original iterated over (isurf × ρ × sza) — that grid
# iteration is trivially addable on top of this single-scenario form.
#
# Usage:
#   julia --project=test test/benchmarks/creategrid_O2Aband_OCORayl.jl
#   SZA=45 ALBEDO=0.3 SIF_ON=0 julia --project=test test/benchmarks/creategrid_O2Aband_OCORayl.jl
# =========================================================================
...
const GRID_YAML_OCO_A = get(ENV, "GRID_YAML_OCO_A",
    joinpath(pkgdir(vSmartMOM), "test", "test_parameters",
             "O2_parameters2_SIF_grid.yaml"))
const GRID_OUTDIR_OCO_A = get(ENV, "GRID_OUTDIR_OCO_A",
    joinpath(pkgdir(vSmartMOM), "test", "benchmarks", "aerosol_exploration_output",
             "grid_O2Aband_OCORayl"))
...
mkpath(GRID_OUTDIR_OCO_A)
@info "OCO O2-A Rayleigh grid driver" GRID_YAML_OCO_A GRID_OUTDIR_OCO_A SIF_ON

parameters = parameters_from_yaml(GRID_YAML_OCO_A)
...
outpath = joinpath(GRID_OUTDIR_OCO_A, "rrs_grid_OCO_O2A_sza$(sza_str)_alb$(alb_str)_$(sif_tag).jld2")
...
@info "OCO O2-A Rayleigh grid driver: wrote $outpath"
