...
#xnSIF = zeros(length(psurf), length(ρ_str), length(sza))

#x_ie = zeros(length(psurf), length(ρ_str), length(sza))
#xn_ie = zeros(length(psurf), length(ρ_str), length(sza))
#x0_ie = zeros(length(psurf), length(ρ_str), length(sza))
#xn0_ie = zeros(length(psurf), length(ρ_str), length(sza))
#x1_ie = zeros(length(psurf), length(ρ_str), length(sza))
#xn1_ie = zeros(length(psurf), length(ρ_str), length(sza))
...
#xSIF_0 = zeros(length(psurf), length(ρ_str), length(sza))
#xnSIF_0 = zeros(length(psurf), length(ρ_str), length(sza))

#x_ie_0 = zeros(length(psurf), length(ρ_str), length(sza))
#xn_ie_0 = zeros(length(psurf), length(ρ_str), length(sza))
#x0_ie_0 = zeros(length(psurf), length(ρ_str), length(sza))
#xn0_ie_0 = zeros(length(psurf), length(ρ_str), length(sza))
#x1_ie_0 = zeros(length(psurf), length(ρ_str), length(sza))
#xn1_ie_0 = zeros(length(psurf), length(ρ_str), length(sza))
coAdd_N = 36
...
# Window 1, no noise
w1_geo_SIF0_itp = interpolate(w1_xSIF0, BSpline(Cubic(Line(OnGrid()))))    
w1_geo_SIF0_sitp = scale(w1_geo_SIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w1_geo_corrSIF0_itp = interpolate(w1_xSIF0_corr, BSpline(Cubic(Line(OnGrid()))))    
w1_geo_corrSIF0_sitp = scale(w1_geo_corrSIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w1_geo_SIF1_itp = interpolate(w1_xSIF1, BSpline(Cubic(Line(OnGrid()))))    
w1_geo_SIF1_sitp = scale(w1_geo_SIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w1_geo_corrSIF1_itp = interpolate(w1_xSIF1_corr, BSpline(Cubic(Line(OnGrid()))))    
w1_geo_corrSIF1_sitp = scale(w1_geo_corrSIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
# Window 1, noisy
w1n_geo_SIF0_itp = interpolate(w1_xnSIF0, BSpline(Cubic(Line(OnGrid()))))    
w1n_geo_SIF0_sitp = scale(w1n_geo_SIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w1n_geo_corrSIF0_itp = interpolate(w1_xnSIF0_corr, BSpline(Cubic(Line(OnGrid()))))    
w1n_geo_corrSIF0_sitp = scale(w1n_geo_corrSIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w1n_geo_SIF1_itp = interpolate(w1_xnSIF1, BSpline(Cubic(Line(OnGrid()))))    
w1n_geo_SIF1_sitp = scale(w1n_geo_SIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w1n_geo_corrSIF1_itp = interpolate(w1_xnSIF1_corr, BSpline(Cubic(Line(OnGrid()))))    
w1n_geo_corrSIF1_sitp = scale(w1n_geo_corrSIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
# Window 2, no noise
w2_geo_SIF0_itp = interpolate(w2_xSIF0, BSpline(Cubic(Line(OnGrid()))))    
w2_geo_SIF0_sitp = scale(w2_geo_SIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w2_geo_corrSIF0_itp = interpolate(w2_xSIF0_corr, BSpline(Cubic(Line(OnGrid()))))    
w2_geo_corrSIF0_sitp = scale(w2_geo_corrSIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w2_geo_SIF1_itp = interpolate(w2_xSIF1, BSpline(Cubic(Line(OnGrid()))))    
w2_geo_SIF1_sitp = scale(w2_geo_SIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w2_geo_corrSIF1_itp = interpolate(w2_xSIF1_corr, BSpline(Cubic(Line(OnGrid()))))    
w2_geo_corrSIF1_sitp = scale(w2_geo_corrSIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
# Window 1, noisy
w2n_geo_SIF0_itp = interpolate(w2_xnSIF0, BSpline(Cubic(Line(OnGrid()))))    
w2n_geo_SIF0_sitp = scale(w2n_geo_SIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w2n_geo_corrSIF0_itp = interpolate(w2_xnSIF0_corr, BSpline(Cubic(Line(OnGrid()))))    
w2n_geo_corrSIF0_sitp = scale(w2n_geo_corrSIF0_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w2n_geo_SIF1_itp = interpolate(w2_xnSIF1, BSpline(Cubic(Line(OnGrid()))))    
w2n_geo_SIF1_sitp = scale(w2n_geo_SIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
  
w2n_geo_corrSIF1_itp = interpolate(w2_xnSIF1_corr, BSpline(Cubic(Line(OnGrid()))))    
w2n_geo_corrSIF1_sitp = scale(w2n_geo_corrSIF1_itp, 500.0:250:1000, 0:0.05:0.7, 0.35:0.05:1.0)
...
t_w1_geo_SIF0 = zeros(46,length(lon),length(lat))
t_w2_geo_SIF0 = zeros(46,length(lon),length(lat))
t_w1_geo_corrSIF0 = zeros(46,length(lon),length(lat))
t_w2_geo_corrSIF0 = zeros(46,length(lon),length(lat))
for t=1:46
    alb_850_t=alb_850[t,:,:]
    ...
    for i = 1:length(lon)
        ...
        ind = findall(x -> !isa(x, Missing) && 0.0<x<0.7, alb_850_t[i,:])
        #@show ind
        ...
        if length(ind)>0
            for ctr=1:length(ind) # = 1:length(lat)
                ...
                if(cosd(pSZA)>=0.35)
                    ...
                    t_w1_geo_SIF0[t,i,j] = w1_geo_SIF0_sitp(1000., palb, cosd(pSZA))
                    t_w1_geo_corrSIF0[t,i,j] = w1_geo_corrSIF0_sitp(1000., palb, cosd(pSZA))
                    t_w2_geo_SIF0[t,i,j] = w2_geo_SIF0_sitp(1000., palb, cosd(pSZA))
                    t_w2_geo_corrSIF0[t,i,j] = w2_geo_corrSIF0_sitp(1000., palb, cosd(pSZA))
...
for t=1:46
    l = @layout [a1 a2; b1 b2]
    p1 = heatmap(lon, lat, t_w1_geo_SIF0[t,:,:]', size=(800,400), grid=true, xlabel="", ylabel="Lon [ᵒ]")
    # title=L"$\mathrm{Uncorr.}\,\mathrm{SIF\,\,retr.} (\hat{\mathrm{SIF}}=0.34\,\mathrm{W}/\mathrm{m}^2/\mathrm{sr}/\mu\mathrm{m})$")
    p2 = heatmap(lon, lat, t_w2_geo_SIF0[t,:,:]', size=(800,400), grid=true, xlabel="", ylabel="")
    #heatmap(ρ, sza, w2_xnSIF1_0[3,:,:]', xlabel="", ylabel="")
    # title=L"$\mathrm{Uncorr.}\,\mathrm{SIF\,\,retr.} (\hat{\mathrm{SIF}}=0.20\,\mathrm{W}/\mathrm{m}^2/\mathrm{sr}/\mu\mathrm{m})$")
    p3 = heatmap(lon, lat, t_w1_geo_corrSIF0[t,:,:]', size=(800,400), grid=true, xlabel="Lat [ᵒ]", ylabel="Lon [ᵒ]")
    #heatmap(ρ, sza, w1_xnSIF1_corr_0[3,:,:]', xlabel="Albedo", ylabel="SZA [ᵒ]") 
    p4 = heatmap(lon, lat, t_w2_geo_corrSIF0[t,:,:]', size=(800,400), grid=true, xlabel="Lat [ᵒ]", ylabel="")
    ...
    plot(p1, p2, p3, p4, layout = l, legend = false, title = [title1a*"\n"*title1b title2a*"\n"*title2b title3a*"\n"*title3b title4a*"\n"*title4b], titlefont = font(8))
    savefig("globmap_SIF0_retr"*string(t)*".png")
end
