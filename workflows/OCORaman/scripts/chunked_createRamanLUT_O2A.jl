...
const DEFAULT_CHUNK_TAG =
    "sza$(index_tag(SZA_IDXS, length(FULL_SZA_AXIS)))_alb$(index_tag(ALBEDO_IDXS, length(FULL_ALBEDO_AXIS)))"
const CHUNK_TAG = get(ENV, "CHUNK_TAG", DEFAULT_CHUNK_TAG)
const OUT_NC = get(ENV, "OUT_NC",
    joinpath(OUTDIR, "RamanLUT_O2A_float32_gpu_$(PSURF_TAG)_$(CHUNK_TAG).nc"))
