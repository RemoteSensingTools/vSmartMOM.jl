...
const YAML_PATH = joinpath(OUTDIR, "current_flat_noabs_cpu.yaml")
...
function metadata_lines(params, model, rs, ν, effT)
    ...
    rho_cab = 2γ_cab / (1 + γ_cab)
    rho_ray = 2γ_ray / (1 + γ_ray)
    raw_sum = sum(rs.ϖ_λ₁λ₀)
    ...
    return [
        ...
        "rho_cab_internal=$rho_cab",
        "rho_ray_internal=$rho_ray",
        "varpi_cab_model=$(model.ϖ_Cabannes[1])",
        "varpi_cab_rs=$(rs.ϖ_Cabannes[1])",
        "rrs_weight_sum_before_rt=$raw_sum",
        "n_raman=$(rs.n_Raman)",
...
function run_case(albedo)
    ...
    rs, F0, effT = make_rrs(FT, n_pol, ν, model)
    src = CoreRT.SolarBeam(F₀ = F0) +
          CoreRT.SurfaceSIF(SIF₀ = zeros(FT, n_pol, length(ν)))

    rrs_result = CoreRT.rt_run_test(rs, model, [1]; sources = src)
    ...
    nors_result = CoreRT.rt_run_test(nors, model, [1]; sources = src)
    ...
    @info "wrote current flat/noabs case" albedo rrs_path nors_path
end
