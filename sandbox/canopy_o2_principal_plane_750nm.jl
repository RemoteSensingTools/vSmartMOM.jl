# =================================================================
# Canopy principal-plane reflectance slice at 750 nm
#
# Same one-layer planophile setup as sandbox/canopy_o2_brdf_750nm.jl,
# but only samples the solar principal plane and writes a simple 2D plot of Stokes-I
# reflectance versus signed view zenith angle.
#
...
# Run with `julia --project=docs sandbox/canopy_o2_principal_plane_750nm.jl`.
...
# -- 3) One-layer PROSPECT canopy with planophile LAD ---------------
...
println("Principal-plane R(I) min/max: ", extrema(R_principal))
...
ax = Axis(fig[1, 1];
    ...
    title = "Planophile-LAD one-layer canopy at 750 nm, principal plane (SZA = 30 deg)",
)
lines!(ax, signed_vza, R_principal; color = :darkgreen, linewidth = 2.5,
       label = "Canopy reflectance")
...
out_png = joinpath(@__DIR__, "canopy_o2_principal_plane_750nm.png")
save(out_png, fig)
println("Saved principal-plane plot -> ", out_png)
fig
