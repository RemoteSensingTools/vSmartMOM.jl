...
const TRUTH_ROOT = joinpath(DATA_ROOT, "truth_map")
const AEROSOL_ROOT = joinpath(TRUTH_ROOT, "aerosol_chunked")
const INSTRUMENT_ROOT = joinpath(WORKFLOW_ROOT, "inversion", "instrument")
const INVERSION_ROOT = joinpath(DATA_ROOT, "inversion")
...
function configured_email()
    ...
    email = try
        strip(read(`git -C $REPO_ROOT config --get user.email`, String))
    catch
        ""
    end
...
function run_stage(name, relative_script; environment=Dict{String,String}())
    script = joinpath(WORKFLOW_ROOT, relative_script)
    ...
    command = `$(Base.julia_cmd()) --project=$REPO_ROOT $script`
