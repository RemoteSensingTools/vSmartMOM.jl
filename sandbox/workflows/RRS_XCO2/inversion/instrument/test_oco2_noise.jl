...
@testset "OCO-2 L1B ATBD noise model" begin
    ...
    data_root = normpath(get(ENV, "RRS_XCO2_DATA_ROOT", workflow_root))
    coefficient_path = get(
        ENV, "OCO_SNR_COEFFICIENTS",
        joinpath(data_root, "inversion", "instrument",
                 "representative_snr_coefficients.nc"))
